...
/*                min_lev = MAX(min_lev, 1);
    ...
    if (len < sizeof (buf) && max_lev == ADMLVL_IMMORT)
...
ACMD(do_toggle) {
    ...
    switch (toggle) {
        ...
        case SCMD_TRACK:
            ...
            break;
        case SCMD_BUILDWALK:
            if (GET_LEVEL(ch) < ADMLVL_BUILDER) {
...
ACMD(do_whois) {
    ...
    if (!(GET_LEVEL(victim) < ADMLVL_IMMORT) || (GET_LEVEL(ch) >= GET_LEVEL(victim))) {
    ...
    if (!got_from_file && victim->desc != NULL && GET_LEVEL(ch) >= ADMLVL_GOD) {
